...
- **DateTimes serialize as UTC (with `Z`).** `Program.cs` registers `UtcDateTimeJsonConverter` (in `CustomerService.Api/Json/`) — EF Core returns `Kind=Unspecified` after a DB round-trip, which would otherwise emit timezone-naive strings that the frontend parses as local time. All `*Utc` columns are written from `DateTime.UtcNow`; keep it that way (never store local time in a `*Utc` column).
- **ML wiring:** `CaseService.CreateAsync` calls `IPriorityPredictor` (backend `OnnxPriorityPredictor`, falls back to `RuleBasedPriorityPredictor` if the `.onnx` is missing). Model input = 4 floats `[categoryId, priorCaseCount, daysSinceLastContact, sentiment]`; output order `[Low, Medium, High]`. `sentiment` is a lexicon-based score in [-1, 1] computed from the description by `RuleBasedPriorityPredictor.SentimentScore` (mirrors `sentiment_score` in `ml/train_model.py`) — it replaces the old binary `hasComplaintKeyword` flag. Retrain via `ml/train_model.py` (opset 17, integer labels 0/1/2).
...
## Ponytail mode (lazy senior dev) — applies to all coding in this repo
