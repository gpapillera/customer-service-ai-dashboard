...
public class FakeCaseService : ICaseService
{
    public Task<IReadOnlyList<CaseDto>> GetAllAsync(
        ...
        bool overdue = false, string? assignedToUserId = null, string? callerRole = null, string? callerUserId = null, bool unassigned = false, string? assignedToUserIdFilter = null) =>
    ...
    public Task<IReadOnlyList<ConversationSummaryDto>> GetAllConversationsAsync(string viewerUserId) =>
        Task.FromResult<IReadOnlyList<ConversationSummaryDto>>(new List<ConversationSummaryDto>());
}
